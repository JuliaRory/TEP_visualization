...
ECONOMI_SURVEY_PATH = os.path.join("resources", "survey_econoMI.json")

os.environ["QT_QPA_PLATFORM_PLUGIN_PATH"] = r".\venv\Lib\site-packages\PyQt5\Qt5\plugins"


def parse_args():
    parser = argparse.ArgumentParser(description="Run econoMI questionnaire window.")
    parser.add_argument(
        "--config",
        default=ECONOMI_SURVEY_PATH,
        help="Path to econoMI survey JSON config.",
    )
    parser.add_argument(
        "--output-dir",
