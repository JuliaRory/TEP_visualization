...
class TopoplotPanel(QFrame):
    ...
    def _init_ui(self):
        ...
        self.setObjectName("topoplot_panel")    # для привязки стиля
    ...
    def _setup_ui(self):
        print('hi')
        # self.figure = TopoPlot(self, single_w=0.7*self.width(), single_h=300, w=self.width(), h=self.height(), Fs=self.params["SPEED"]['Fs'])
        # self.figure.setAttribute(Qt.WA_TransparentForMouseEvents, True)
        



        # Отрисовать: передаём либо values + evoked.info, либо сам evoked
        self.topo = TopoCanvas(self)
